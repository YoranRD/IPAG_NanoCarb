...
class Camera(object):
    ...
    def get_feature(self, name):
        ...
        if ntype in ("Enumeration", "String", "StringReg"):
            ...
            return self.dev.get_boolean_feature_value(name)
        else:
    ...
    def pop_frame(self):
        while True: #loop in python in order to allow interrupt, have the loop in C might hang
            timestamp, frameId, frame = self.try_pop_frame()
            if frame is None:
                time.sleep(0.001)
            else:
                return timestamp, frameId, frame
    ...
    def try_pop_frame(self):
        ...
        if buf:
            ...
            return buf.get_system_timestamp(), buf.get_frame_id(), frame
        else:
