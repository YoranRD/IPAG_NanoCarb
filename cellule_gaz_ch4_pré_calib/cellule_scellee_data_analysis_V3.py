...
"""
Programe pour analyser les datas de la manip sur la cuve CH4 scellée pré calibrée

1) extraire datas

2) graph la concentration
...
def get_absorbance():
    ...
    #intensity, filtered_intensity, max_pic, max_pic_index = process_intensity_data(l_io, l_onde)
...
def find_local_peaks(wavelengths, absorbance, wavelengths_pics_theoriques):
    local_peaks = []
    threshold = 0.9  # Seuil minimal d'absorbance
...
def find_absorption_lines(wavelengths, intensity):
    local_minima = scipy.signal.argrelmin(intensity)[0]
    absorption_lines = []

    for index in local_minima:
        wavelength = wavelengths[index]
        intensity_value = intensity[index]
        absorption_lines.append((wavelength, intensity_value))

    return absorption_lines

# Exemple d'utilisation
wavelengths = [6000, 6050, 6100, 6150, 6200]
intensity = [1.2, 1.1, 0.9, 1.0, 1.3]

lines = find_absorption_lines(wavelengths, intensity)
for line in lines:
    print("Raie d'absorption :")
    print("Longueur d'onde :", line[0])
    print("Intensité :", line[1])



def process_intensity_data(intensity,wavelength):
    # Trouver la raie dabsorption la plus haute
    # Chercher les indices des sommets de toutes les raies dabsorption
    
    # Convertir les listes en tableaux numpy sinon erreur plus loin dans manipulation de données
    intensity = np.array(intensity, dtype=np.float64)
    
    # Trouver les pics dans l'intensité
    peak_indices, _ = scipy.signal.find_peaks(intensity)
    max_pic_index = peak_indices[np.argmax(intensity[peak_indices])]
    max_pic = intensity[max_pic_index]
    ...
    for i in peak_indices:
        print('long onde', wavelength[i], 'pics associés', filtered_intensity[i]) 
        print('max pic', max_pic)
        print('long onde peak_indice', wavelength[max_pic_index], ' max peak ind   :  ', max_pic_index)
    return intensity, filtered_intensity, max_pic, max_pic_index
...
def get_concentration(wavelengths_pics_theoriques,extinction_coefficients):
    ...
    for peak in local_peaks:
        ...
        print("Pic local d'absorption le plus proche de :",wavelengths_pics_theoriques[i])
